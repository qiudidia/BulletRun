...
@export var obstacle_count: int = 20
@export var map_type: int = 1  # BOT模式风格（明亮干净）
@export var barrel_count: int = 6
@export var shop_zone_center: Vector2 = Vector2(600, 600)
@export var shop_zone_radius: float = 200.0
...
func _add_floor_detail() -> void:
	...
	for i in range(100):
		...
		var length: float = rng.randf_range(20, 60)
		var width: float = rng.randf_range(2, 5)
	...
	for i in range(50):
		var half_map: Vector2 = map_size * 0.5
		var pos: Vector2 = Vector2(rng.randf_range(-half_map.x, half_map.x), rng.randf_range(-half_map.y, half_map.y))
		var s: float = rng.randf_range(6, 12)
...
func _create_obstacle_at(rng: RandomNumberGenerator, pos: Vector2) -> void:
	var base_s: float = rng.randf_range(50, 100)
	var ratio: float = rng.randf_range(0.6, 1.4)
	var size: Vector2 = Vector2(base_s, base_s * ratio)
	size.x = clamp(size.x, 45, 120)
	size.y = clamp(size.y, 45, 120)
